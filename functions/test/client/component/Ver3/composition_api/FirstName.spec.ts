...
import { methods, watchers, handleOnMounted } from '@component/Ver3/composition_api/FirstName';
...
describe('watchers', () => {
  ...
  describe('inputValue', () => {
    ...
    test('called countUp method', async () => {
      wrapper.vm.inputValue = 'Hello World';
      await wrapper.vm.$nextTick();
...
describe('onMounted', () => {
  ...
  beforeEach(async () => {
    component = defineComponent({
      template: '<p>Hello World</p>',
      setup(props){
        const inputValue: Ref<string> = ref('hello world');
  ...
  test('resets inputValue value to blank', () => {
